...
Function Invoke-DNSManualCleanUp {
    ...
    Process {
        ...
        Foreach ($Zone in $ZoneName) {
            $records += Get-DnsServerResourceRecord -ZoneName $Zone -RRType 'A' -CimSession $s | Where-NotSystem
...
Function Get-MyIpAddress {
    ...
    Try {
        ...
        $ip = Invoke-WebRequest -UseBasicParsing -Proxy $null -Uri $uri
...
function Test-IPAddressIsValid {
    ...
    if ([ipaddress] $IP) { $true } else { $false }
}
...
function Get-GeoIPInfo {
    param (
        ...
        [ValidateSet('Country', 'CityPlus', 'Insights')]
        [string]$ServiceType = 'Insights',
    ...
    # We're licensed for any, and have a $25 top-up when we've used 75% of our queries.
    # We're using the Insights service, which is the most detailed. $25 = 12,500 queries
    $baseUrls = @{
        "Country"  = "https://geoip.maxmind.com/geoip/v2.1/country/"
        "CityPlus" = "https://geoip.maxmind.com/geoip/v2.1/city/"
        "Insights" = "https://geoip.maxmind.com/geoip/v2.1/insights/"
    }

    $url = $baseUrls[$ServiceType] + $IPAddress.ToString()

    $accountID = $Credential.UserName
    $licenseKey = $Credential.GetNetworkCredential().Password

    # Basic authentication
    $authInfo = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes(('{0}:{1}' -f $accountID, $licenseKey)))
    ...
    try {
        ...
        Write-Error "Failed to query GeoIP service: $_"
    }
}


function New-MxToolBoxQuery {

    Param (
        [Parameter( Mandatory, HelpMessage = 'Your MXToolBox API key goes here')]
        [ValidateScript({
                If (($_).Length -ne 36) {
        ...
        [Parameter( HelpMessage = 'The domain/IP to query')]
        [ValidateScript({
                If (($_).Length -lt 3) {
        ...
        [Alias('Domain', 'IPAddress')]
        [String]$HostName,

        [ValidateSet('usage', 'blacklist', 'dns', 'smtp', 'spf', 'soa', 'a', 'mx', 'txt', 'ptr', 'http', 'https', 'tcp', 'udp', 'ssl', 'ssh', 'sip', 'rdp', 'imap', 'pop', 'smtp', 'ftp', 'http', 'https', 'tcp', 'udp', 'ssl', 'ssh', 'sip', 'rdp', 'imap', 'pop', 'smtp', 'ftp', IgnoreCase = $false)]
        [String]$QueryType = 'usage'
    )

    Begin {

        # if PTR, then we need to make sure we received an IP address as the host name parameter

        # for all others, test if it's an IP address or a domain name

        $header = @{ 'Authorization' = $MXAPI_key }

        $baseURL = 'https://api.mxtoolbox.com/api/v1/lookup/'
    }

    Process {

        if ($QueryType -eq 'usage') {

            $uri = $baseURL -Replace 'lookup', 'usage'
        }
        else {

            if ($null -eq $HostName) {

                Write-Error "Please provide a valid domain name or IP address"
                return
            }

            $uri = $baseURL + $QueryType + '/' + $HostName
        }
        ...
        try {
            ...
            Write-Error "Failed to query MXToolBox service: $_"
        }

        if ($QueryType -eq 'usage') {
            Write-Host -BackgroundColor Green -ForegroundColor Black "Stats: DnsRequests = $($response.DnsRequests); DnsMax = $($response.DnsMax); Queries remaining = $($response.DnsMax - $response.DnsRequests)"
        }
        else {
